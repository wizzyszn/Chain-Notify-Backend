...
class NotificationService {
  ...
  formatTransactionMessage(type: 'sent' | 'received', amount: number, from: string, to: string): string {
...
const validateTransaction = (transaction: any): transaction is TransactionData => {
  return (
    transaction &&
...
// Webhook handler
const quicknodeWebHook: RequestHandler = async (req: Request, res: Response): Promise<void> => {
  const notificationService = new NotificationService();

  try {
    const transactions = req.body;
    
    const results = await Promise.all(transactions.map(async (transaction: TransactionData) => {
      const parsedData = {
      ...
      const updatePromises = users.map(async (user) => {
        ...
        const notificationMessage = notificationService.formatTransactionMessage(
